...
void	raycaster(t_cub3d *cub3d, double ra, int ray)
{
	...
	if (cub3d->disth < cub3d->distv)
	{
		...
		if (ra < M_PI) // SOUTH wall
			draw_line_textu(HEIGHT * 1.6 / (cub3d->final_d / UNIT), ray, cub3d->S->height - pixel - 1, cub3d->S, cub3d);
		else		 // NORTH WALL
		{
			pixel = tx / UNIT * cub3d->N->height;
			draw_line_textu(HEIGHT * 1.6 / (cub3d->final_d / UNIT), ray, pixel, cub3d->N, cub3d);
		}
	}
	else
	{
		...
		if (ra <= M_PI_2 || ra >= (3 * M_PI_2)) // WEST wall
			draw_line_textu(HEIGHT * 1.6 / (cub3d->final_d / UNIT), ray, pixel, cub3d->E, cub3d);
		else		 // EAST WALL
			draw_line_textu(HEIGHT * 1.6 / (cub3d->final_d / UNIT), ray, cub3d->E->height - pixel - 1, cub3d->W, cub3d);
	}
}
